(function(){
  ...
  function render(cfg){
    ...
    const renderers={
      ...
      participation:()=>section("participation","participation",'<h2 class="section-title">'+esc(cfg.participation?.title||"Confirmă participarea")+'</h2><p style="text-align:center">'+esc(cfg.participation?.description||"")+'</p><form id="registrationForm" class="upload-card"><div style="display:grid;gap:14px;text-align:left">'+(cfg.participation?.fields?.name!==false?'<label>Nume<select id="guestSelect" name="nume_complet" required><option value="">Se încarcă...</option></select></label>':"")+(cfg.participation?.fields?.participation!==false?'<label>Participare<select name="participa" required><option value="">Alege...</option><option value="da">Da</option><option value="nu">Nu</option></select></label>':"")+(cfg.participation?.fields?.persons!==false?'<label>Număr persoane<input type="number" name="persoane" min="1" value="1"></label>':"")+(cfg.participation?.fields?.products!==false?'<div><strong>Contribuție</strong><div class="product-row"><select name="productId1"><option value="">Alege produsul...</option></select><input name="cantitate1" type="number" min="0" step="0.1" placeholder="Cantitate"></div><div class="product-row"><select name="productId2"><option value="">Alege produsul...</option></select><input name="cantitate2" type="number" min="0" step="0.1" placeholder="Cantitate"></div></div>':"")+(cfg.participation?.fields?.notes!==false?'<label>Observații<textarea name="observatii" rows="3"></textarea></label>':"")+'</div><button class="btn" type="submit">'+esc(cfg.participation?.buttonText||"Confirmă")+'</button></form>'),
      stats:()=>section("stats","stats",'<h2 class="section-title">'+esc(cfg.stats?.title||"Statistici")+'</h2><div class="stats-grid">'+(cfg.stats?.cards||[]).filter(x=>x.enabled!==false).map((x,i)=>'<div class="stat-card"><span id="'+esc(["invited","confirmed","declined","waiting","persons"][i]||("stat"+i))+'">0</span><div>'+esc(x.label||"")+'</div></div>').join("")+'</div>'),
      food:()=>section("food","food",'<h2 class="section-title">'+esc(cfg.food?.title||"Vreau să contribui")+'</h2><p style="text-align:center">'+esc(cfg.food?.description||"")+'</p><div id="foodProgress" class="food-grid">'+(cfg.food?.products||[]).filter(x=>x.enabled!==false).map(x=>'<div class="food-card" data-product-id="'+attr(x.id||"")+'" data-product-name="'+attr(x.name||"")+'"><div class="food-card-icon">'+esc(x.icon||"🍽️")+'</div><h3>'+esc(x.name||"Produs")+'</h3><div class="food-progress-label">PROGRES</div><div class="food-progress-number">0 / '+esc(x.required||0)+' '+esc(x.unit||"")+'</div><div class="food-progress" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0"><div class="food-progress-bar" style="width:0%"></div></div></div>').join("")+'</div>'),
